...
func (suite *SimpleDatatypeSuite) TestTransactionFail() {
	...
	intCounter1, err := ortoo.NewIntCounter("key1", cuid1, tw, nil)
	if err != nil {
	...
	intCounter2, err := ortoo.NewIntCounter("key1", cuid2, tw, nil)
	if err != nil {
		suite.Fail("fail to create intCounter2")
	}

	tw.SetDatatypes(intCounter1, intCounter2)
	if err := intCounter1.DoTransaction("transaction1", func(intCounter ortoo.IntCounterInTxn) error {
	...
	if err := intCounter1.DoTransaction("transaction2", func(intCounter ortoo.IntCounterInTxn) error {
...
func (suite *SimpleDatatypeSuite) TestOneOperationSyncWithTestWire() {
	tw := NewTestWire()
	intCounter1, err := ortoo.NewIntCounter("key1", model.NewNilCUID(), tw, nil)
	if err != nil {
		suite.T().Fatal(err)
	}
	intCounter2, err := ortoo.NewIntCounter("key2", model.NewNilCUID(), tw, nil)
	if err != nil {
		suite.T().Fatal(err)
	}
...
func (suite *SimpleDatatypeSuite) TestPushPullPackSync() {
	intCounter1, err := ortoo.NewIntCounter("key1", model.NewNilCUID(), datatypes.NewDummyWire(), nil)
	if err != nil {
		suite.Fail("fail to create intCounter1")
	}
